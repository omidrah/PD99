...
{
    [AllowAnonymous]
    public partial class ShopCartController : Controller
    {
        IUnitOfWork _uow;//db
        ...
        public ShopCartController(IUnitOfWork uow, IProductService product, IProductGroupService productGroup, IPaymentUniqueNumberService payment
            ...
            , IFeaturesService features, IOrderService orders, IOrderStatusService orderStatus, IOrderDetailsService orderDetails, ICustomerService customer
            , IDoorehService dooreh, ISettingsService settings)
        {
            _uow = uow;
            _products = product;
            _user = user;
        ...
        List<ShowOrderViewModel> getListOrder()
        {
            ...
            List<ShowOrderViewModel> list = CalCulateSabadFromSesstion();
            var orders = list;
            int orderWeight = orders.Sum(m => m.Weight);
            int transferCost = 0;
            if (list.Where(ll => ll.ItemType == "Product" || ll.ItemType == "Package").Any())
            {
                transferCost = _settings.GetAll().FirstOrDefault().PostBasePrice + _settings.GetAll().FirstOrDefault().PostPricePerUnit * orderWeight / 1000;
            }
        ...
        private List<ShowOrderViewModel> CalCulateSabadFromSesstion()
        {
            bool hasOff = false;
                    ...
                    foreach (var item in listShop)
                    {
                        if (item.ItemType == "Product")
                        {
                            ...
                            list.Add(new ShowOrderViewModel()
                            {
                                ...
                                OffPrice = product.Price - product.Price * product.OffPercent / 100 ?? 0,
                        ...
                        if (item.ItemType == "Package")
                        {
                            ...
                            list.Add(new ShowOrderViewModel()
                            {
                                ...
                                OffPrice = package.Cost - package.Cost * package.OffPercent / 100 ?? 0,
                                Weight = package.Weight * item.Count,
                        ...
                        if (item.ItemType == "Dooreh")
                        {
                            ...
                            list.Add(new ShowOrderViewModel()
                            {
                                ...
                                OffPrice = dooreh.Cost - dooreh.Cost * dooreh.OffPercent / 100 ?? 0,
        ...
        public virtual ActionResult Order()
        {
            ...
            int orderWeight = orders.Sum(m => m.Weight);
            int transferCost = _settings.GetAll().FirstOrDefault().PostBasePrice + _settings.GetAll().FirstOrDefault().PostPricePerUnit * orderWeight / 1000;
            ViewBag.TransferCost = transferCost;
        ...
        [Authorize]        
        public virtual ActionResult SelectShiping()
        {
            var curUser = _user.FindById(_user.GetCurrentUserId());
            var sum = CalCulateSabadFromSesstion().Sum(x => x.Sum);
            List<ShowOrderViewModel> list = CalCulateSabadFromSesstion();
            var orders = list;
            int orderWeight = orders.Sum(m => m.Weight);
            int transferCost = 0;
            ...
            // ViewBag.ShipingType = "Post";
            return View(new OrderShipingViewModel { DisplayName = $"{curUser.Name} {curUser.Family}", Address = curUser.Address, SumTotal = sum,  TransferCost = transferCost });
        }

        [Authorize]
        public virtual ActionResult Payment(string ShippingType)
        {
            int userId = _user.GetCurrentUserId();
            string customerEmail = _user.FindById(userId).Email;
            string customerMobile = _user.FindById(userId).PhoneNumber;
            bool hasOff = false;
            if (User.Identity.IsAuthenticated)
            {
             //   var userId = _user.GetCurrentUserId();
           ...
           // var orders = getListOrder();
            int orderWeight = listDetails.Sum(m => m.Weight);
            int transferCost = _settings.GetAll().FirstOrDefault().PostBasePrice + _settings.GetAll().FirstOrDefault().PostPricePerUnit * orderWeight / 1000;
            if(ShippingType =="Peyk")
            {
                transferCost = 0;
            }
            ...
            Order order = new Order()
            {
                ...
                IsFinalized = false,
                Sum = listDetails.Sum(o => o.Sum) + transferCost,
                OrderStatusId = 0
                    ...
                    if (item.ItemType == "Package")
                    {
                        ...
                        foreach (var product in packageProduct)
                        {
                            _orderDetails.Add(new OrderDetail()
                            {
                                OrderedCount = item.Count,
                                OrderID = order.OrderID,
                                Price = product.Products.Price - product.Products.Price*product.Products.OffPercent/100 ?? 0,
            ...
            string email = customerEmail;// "info@parsian-danesh.ir";
            string mobile = customerMobile;// "09123715763";
            string callbackUrl = "http://parsian-danesh.ir/ShopCart/Verify/" + po.PaymentUniqueID;
            ...
            if (curPaymentUniquNumber != null)
            {
                if (!string.IsNullOrEmpty(Request.QueryString["Status"]) && !string.IsNullOrEmpty(Request.QueryString["Authority"]))
                {
                    if (Request.QueryString["Status"].Equals("OK"))
                    {
                        ...
                        int Status = zp.PaymentVerification("4cdd0752-bc9e-4d32-8658-05cf3d3c8425",
        ...
        //public void SavePaymentLog(int OrderID,int )
    }
}